<?php
	// meta keywords
	// meta description
	// view all upcoming stacks button text
	// view stack archive button text
	// i'm attending this stack text
	// cancel attending text
	// i want to attend this stack text
	// requested by text
	// posted by text
	// show steam links
	// IRL stack text
	...
	// types of stacks (irl, online)
	// number of stacks per archive page
	// ajax load more stacks
	// google analytics
?>
...
Based on Plugin Options Starter Kit
...
function posk_add_defaults() {
	/*
	$tmp = get_option('co_options');
    if(($tmp['chk_default_options_db']=='1')||(!is_array($tmp))) {
		delete_option('co_options'); // so we don't have to reset all the 'off' checkboxes too! (don't think this is needed but leave for now)
...
function posk_init(){
	register_setting( 'posk_plugin_options', 'co_options', 'posk_validate_options' );
}
...
function posk_render_form() {
	...
	<div class="wrap">
		...
		<form method="post" action="options.php">
			...
			<?php $options = get_option('co_options'); ?>
			...
			<table class="form-table" id="form-layout">
				...
				<tr>
					...
					<td>
						<label><input name="co_options[wpmenu]" type="checkbox" value="1" <?php if (isset($options['wpmenu'])) { checked('1', $options['wpmenu']); } ?> /> Tick to enable Wordpress Menu instead of hardcoded menu.</label>
					</td>
				</tr>
				<tr>
					<th scope="row">Google +1 Buttons</th>
					<td>
						<label><input name="co_options[plusone]" type="checkbox" value="1" <?php if (isset($options['plusone'])) { checked('1', $options['plusone']); } ?> /> Tick to enable Google +1 Buttons</label>
					</td>
				</tr>
				<tr>
					<th scope="row">Google +1 Count</th>
					<td>
						<label><input name="co_options[plusone_count]" type="checkbox" value="1" <?php if (isset($options['plusone_count'])) { checked('1', $options['plusone_count']); } ?> /> Tick to enable the count bubbles on the +1 buttons. </label>
					</td>
				</tr>
				<tr>
					<th scope="row">Use Widgets</th>
					<td>
						<label><input name="co_options[useWidgets]" type="checkbox" value="1" <?php if (isset($options['useWidgets'])) { checked('1', $options['useWidgets']); } ?> /> Tick to enable widget areas in the lower area. </label>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[homeslides]" value="<?php echo $options['homeslides']; ?>" />
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[slidePostCategory]" value="<?php echo $options['slidePostCategory']; ?>" />
				...
				<tr>
					...
					<td>
						<label><input name="co_options[authorinfo]" type="checkbox" value="1" <?php if (isset($options['authorinfo'])) { checked('1', $options['authorinfo']); } ?> /> Show author info at the bottom of posts. Only shows if the author has filled in their Wordpress profile.</label>
					</td>
				</tr>
				<tr>
					<th scope="row">Show "Listening To" field on posts</th>
					<td>
						<label><input name="co_options[listeningto]" type="checkbox" value="1" <?php if (isset($options['listeningto'])) { checked('1', $options['listeningto']); } ?> /> If ticked, you can set a "Listening To" custom field when posting that will display at the bottom of your published post.</label>
					</td>
				</tr>
				<tr>
					<th scope="row">AJAX Load More Posts</th>
					<td>
						<label><input name="co_options[loadmore]" type="checkbox" value="1" <?php if (isset($options['loadmore'])) { checked('1', $options['loadmore']); } ?> /> Tick to enable a twitter/facebook style 'load more' button instead of pagination. </label>
					</td>
				</tr>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[social_image]" value="<?php echo $options['social_image']; ?>" />
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[fb_ogtype]" value="<?php echo $options['fb_ogtype']; ?>" />
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[fb_admins]" value="<?php echo $options['fb_admins']; ?>" />
						<br /><span style="color:#666666;margin-left:2px;">Used for <a href="https://developers.facebook.com/docs/opengraph/" target="_blank">Facebook sharing</a>.</span>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[fb_appid]" value="<?php echo $options['fb_appid']; ?>" />
						<br /><span style="color:#666666;margin-left:2px;">Used for <a href="https://developers.facebook.com/docs/opengraph/" target="_blank">Facebook sharing</a>.</span>
					</td>
				</tr>
			</table>
			...
			<table class="form-table" id="form-meta">
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[meta_author]" value="<?php echo $options['meta_author']; ?>" />
					</td>
				</tr>
				<tr>
					<th scope="row">Meta Description</th>
					<td>
						<input type="text" size="57" name="co_options[meta_description]" value="<?php echo $options['meta_description']; ?>" />
					</td>
				</tr>
				<tr>
					<th scope="row">Meta Keywords</th>
					<td>
						<input type="text" size="57" name="co_options[meta_keywords]" value="<?php echo $options['meta_keywords']; ?>" />
					</td>
				</tr>
			</table>
			...
			<table class="form-table" id="form-text">
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[searchplaceholder]" value="<?php echo $options['searchplaceholder']; ?>" />
				...
				<tr>
					...
					<td>
						<?php
							$args = array("textarea_name" => "co_options[homecontent]");
							wp_editor( $options['homecontent'], "co_options[homecontent]", $args );
						?>
				...
				<tr>
					...
					<td>
						<?php
							$args = array("textarea_name" => "co_options[footertext]");
							wp_editor( $options['footertext'], "co_options[footertext]", $args );
						?>
				...
				<tr>
					...
					<td>
						<?php
							$args = array("textarea_name" => "co_options[commentswarn]");
							wp_editor( $options['commentswarn'], "co_options[commentswarn]", $args );
						?>
			...
			<table class="form-table" id="form-other">
				...
				<tr>
					...
					<td>
						<input type="text" size="57" name="co_options[googleua]" value="<?php echo $options['googleua']; ?>" />
